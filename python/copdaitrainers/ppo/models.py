...
class PPOModel(LearningModel):
    def __init__(self, params, brain_name, lr=1e-4, h_size=128, epsilon=0.2, beta=1e-3, max_step=5e6,
        ...
        """
        LearningModel.__init__(self, m_size, normalize, use_recurrent, params, brain_name)
        if num_layers < 1:
            num_layers = 1
        self.last_reward, self.new_reward, self.update_reward = self.create_reward_encoder()
        if params.get_action_space_type(brain_name) == "continuous":
